import { CommonInstructionTopicName, Datum, DatumFilter, Instruction, InstructionStates, } from "solarnetwork-api-core/lib/domain/index.js";
import { Logger as log } from "solarnetwork-api-core/lib/util/index.js";
import { HttpContentType, HttpHeaders, HttpMethod, SolarQueryApi, } from "solarnetwork-api-core/lib/net/index.js";
...
class ControlToggler {
    ...
    #getActiveInstruction(data) {
        ...
        if (instruction !== undefined) {
            log.debug("Active instruction %d for node %d found in state %s (set control %s to %s)", instruction.id, this.nodeId, instruction.state, controlId, instruction.parameters !== undefined
    ...
    value(desiredValue) {
        ...
        if (pendingState === InstructionStates.Queued &&
            ...
            log.debug("Canceling node %d pending control %s to %s request %d", this.nodeId, this.controlId, pendingValue, this.#lastKnownInstruction.id);
        ...
        if (currentValue !== desiredValue && pendingValue !== desiredValue) {
            log.debug("Request node %d to change control %s to %d", this.nodeId, this.controlId, desiredValue);
            ...
            if (cancel) {
                enqueue = cancel.then(() => {
                    this.#lastKnownInstruction = undefined;
                    return this.#fetch(HttpMethod.POST, queueInstructionUrl);
                });
            }
            else {
                enqueue = this.#fetch(HttpMethod.POST, queueInstructionUrl);
            }
            enqueue
                .then((instr) => {
                ...
                .catch((error) => {
                log.error("Error updating node %d control toggler %s: %s", this.nodeId, this.controlId, error.status);
    ...
    update() {
        ...
        reqs[0] = this.#fetch(HttpMethod.GET, mostRecentUrl);
        ...
        reqs[1] = this.#fetch(HttpMethod.GET, viewPendingUrl);
        ...
        if (lastKnownInstr?.instructionState &&
            ...
            reqs[2] = this.#fetch(HttpMethod.GET, viewInstructionUrl);
        }
        return Promise.all(reqs)
            ...
            if (newValue !== currValue || execInstruction) {
                log.debug("Current node %d control %s value is %s", this.nodeId, this.controlId, newValue !== undefined ? newValue : "N/A");
            ...
            .catch((error) => {
            log.error("Error querying node %d control toggler %s status: %s", this.nodeId, this.controlId, error.status);
            this.#notifyDelegate(error);
        });
    }
    /**
...
//# sourceMappingURL=controlToggler.js.map