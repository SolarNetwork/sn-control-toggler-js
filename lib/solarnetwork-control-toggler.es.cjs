// https://github.com/SolarNetwork/sn-control-toggler-js Version 2.1.2-dev.0. Copyright 2025 SolarNetwork Foundation.
'use strict';
...
const InstructionActiveStates = new Set([
    domain.InstructionStates.Queuing,
    domain.InstructionStates.Queued,
    domain.InstructionStates.Received,
    domain.InstructionStates.Executing,
]);
/**
...
const InstructionFinishedStates = new Set([
    domain.InstructionStates.Completed,
    domain.InstructionStates.Declined,
]);
/**
 * Extension to Datum class with a specific `val` property.
 */
class ControlDatum extends domain.Datum {
    /** The control value. */
    val;
    constructor(info) {
        super(info);
...
class ControlToggler {
    ...
    constructor(api, auth, nodeId, controlId, queryApi) {
        ...
        this.#queryApi = queryApi || new net.SolarQueryApi(api.environment);
        this.#queryAuth = queryApi
            ? new net.AuthorizationV2Builder(auth.tokenId, queryApi.environment).key(auth.key())
            : auth;
    }
    #notifyDelegate(error) {
        ...
        if (callback !== undefined) {
            try {
                ...
                util.Logger.error("Error in callback: %s", callbackError);
            }
        }
    }
    /**
    ...
    #getActiveInstruction(data) {
        ...
        const instruction = data.reduce((prev, curr) => {
            if (curr.topic ===
                domain.CommonInstructionTopicName.SetControlParameter &&
        ...
        if (instruction !== undefined) {
            util.Logger.debug("Active instruction %d for node %d found in state %s (set control %s to %s)", instruction.id, this.nodeId, instruction.state, controlId, instruction.parameters !== undefined
                ? instruction.parameters[0].value
                : null);
            return new domain.Instruction(instruction);
        }
        return undefined;
    }
    /**
    ...
    #mostRecentValue(controlDatum, instruction) {
        if (!instruction ||
            domain.InstructionStates.Declined.equals(instruction.state)) {
    ...
    #fetch(method, url, auth) {
        ...
        if (method !== net.HttpMethod.GET) {
            ...
            if (queryIndex) {
                ...
                contentType = net.HttpContentType.FORM_URLENCODED_UTF8;
        ...
        if (auth.signingKeyValid) {
            ...
            if (contentType) {
                ...
                headers[net.HttpHeaders.CONTENT_TYPE] = contentType;
            }
            headers[net.HttpHeaders.AUTHORIZATION] = auth.buildWithSavedKey();
    ...
    value(desiredValue) {
        ...
        /* !!!!!
           ...
           !!!!! */
        if (pendingState === domain.InstructionStates.Queued &&
            pendingValue != desiredValue &&
            this.#lastKnownInstruction) {
            // cancel the pending instruction
            util.Logger.debug("Canceling node %d pending control %s to %s request %d", this.nodeId, this.controlId, pendingValue, this.#lastKnownInstruction.id);
            const cancelInstructionUrl = this.#api.updateInstructionStateUrl(this.#lastKnownInstruction.id, domain.InstructionStates.Declined);
            cancel = this.#fetch(net.HttpMethod.POST, cancelInstructionUrl, this.#auth);
            this.#lastKnownInstruction = undefined;
            pendingState = undefined;
            pendingValue = undefined;
        }
        if (currentValue != desiredValue && pendingValue != desiredValue) {
            util.Logger.debug("Request node %d to change control %s to %d", this.nodeId, this.controlId, desiredValue);
            const queueInstructionUrl = this.#api.queueInstructionUrl(domain.CommonInstructionTopicName.SetControlParameter, [{ name: this.controlId, value: String(desiredValue) }], this.nodeId);
            if (cancel) {
                enqueue = cancel.then(() => {
                    this.#lastKnownInstruction = undefined;
                    return this.#fetch(net.HttpMethod.POST, queueInstructionUrl, this.#auth);
                });
            }
            else {
                enqueue = this.#fetch(net.HttpMethod.POST, queueInstructionUrl, this.#auth);
            }
            enqueue
                .then((instr) => {
                this.#lastKnownInstruction = new domain.Instruction(instr);
                this.#notifyDelegate();
                if (this.#timer) {
                    this.stop();
                ...
                .catch((error) => {
                util.Logger.error("Error updating node %d control toggler %s: %s", this.nodeId, this.controlId, error.status);
    ...
    update() {
        ...
        reqs[0] = this.#fetch(net.HttpMethod.GET, mostRecentUrl, this.#queryAuth);
        ...
        reqs[1] = this.#fetch(net.HttpMethod.GET, viewPendingUrl, this.#auth);
        ...
        if (lastKnownInstr?.instructionState &&
            ...
            reqs[2] = this.#fetch(net.HttpMethod.GET, viewInstructionUrl, this.#auth);
        }
        return Promise.all(reqs)
            ...
            const execInstruction = results.length > 2 && results[2]
                ? new domain.Instruction(results[2])
                : undefined;
            ...
            if (newValue !== currValue || execInstruction) {
                util.Logger.debug("Current node %d control %s value is %s", this.nodeId, this.controlId, newValue !== undefined ? newValue : "N/A");
            ...
            .catch((error) => {
            util.Logger.error("Error querying node %d control toggler %s status: %s", this.nodeId, this.controlId, error.status);
            this.#notifyDelegate(error);
        });
    }
    /**
...
exports.ControlDatum = ControlDatum;
exports.ControlToggler = ControlToggler;
//# sourceMappingURL=solarnetwork-control-toggler.es.cjs.map
