...
import { AuthorizationV2Builder, HttpContentType, HttpHeaders, HttpMethod, InstructionState, InstructionStates, Logger, NodeDatumUrlHelper, dateParser } from 'solarnetwork-api-core';
  ...
  */
   ...
   */
		...
		value: function handleRequestAuth(request$$1, method, url, contentType) {
			...
			request$$1.setRequestHeader(HttpHeaders.X_SN_DATE, this.authBuilder.requestDateHeaderValue);
			request$$1.setRequestHeader(HttpHeaders.AUTHORIZATION, this.authBuilder.buildWithSavedKey());
		}
   ...
   */
		...
		value: function deferJsonRequestWithAuth(q, method, url) {
			...
			var req = request(queryIndex >= 0 ? url.substring(0, queryIndex) : url).mimeType(HttpContentType.APPLICATION_JSON).on('beforesend', function (request$$1) {
