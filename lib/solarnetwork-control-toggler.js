// https://github.com/SolarNetwork/sn-control-toggler-js Version 0.0.1-dev.0. Copyright 2017 Matt Magoffin.
 ...
 * @private
 */
var InstructionActiveStates = new Set([solarnetworkApiCore.InstructionStates.Queued, solarnetworkApiCore.InstructionStates.Received, solarnetworkApiCore.InstructionStates.Executing]);
 ...
 * @private
 */
var InstructionFinishedStates = new Set([solarnetworkApiCore.InstructionStates.Completed, solarnetworkApiCore.InstructionStates.Declined]);
 ...
 *                                              will be created using the `environment` from `urlHelper`
  */
	function ControlToggler(urlHelper, authBuilder, controlId, queryUrlHelper) {
		classCallCheck(this, ControlToggler);
   ...
   */
		this.authBuilder = authBuilder || new solarnetworkApiCore.AuthorizationV2Builder(null, urlHelper ? urlHelper.environment : undefined);
   ...
   */
		this.queryUrlHelper = queryUrlHelper || new solarnetworkApiCore.NodeDatumUrlHelper(urlHelper.environment);
		...
		value: function notifyDelegate(error) {
			...
			if (callback !== undefined) {
				try {
					...
					solarnetworkApiCore.Logger.error('Error in callback: %s', callbackError);
				}
			}
		}
   ...
   */
		...
		value: function getActiveInstruction(data) {
			...
			if (instruction !== undefined) {
				solarnetworkApiCore.Logger.debug('Active instruction for %d found in state %s (set control %s to %s)', this.instructionUrlHelper.nodeId, instruction.state, controlId, instruction.parameters[0].value);
			}
			return instruction;
		}
   ...
   */
		...
		value: function lastKnownInstructionState() {
			...
			return lastKnownInstruction === undefined ? undefined : solarnetworkApiCore.InstructionState.valueOf(lastKnownInstruction.state);
		}
   ...
   */
		...
		value: function currentRefreshMs() {
			return InstructionActiveStates.has(this.lastKnownInstructionState()) ? this.pendingRefreshMs : this.refreshMs;
		}
   ...
   */
		...
		value: function mostRecentValue(controlDatum, instruction) {
			if (!instruction || solarnetworkApiCore.InstructionStates.Declined.equals(instruction.status)) {
			...
			var statusDate = solarnetworkApiCore.dateParser(controlDatum.created);
			var instructionDate = solarnetworkApiCore.dateParser(instruction.created);
   ...
   */
		...
		value: function handleRequestAuth(request$$1, method, url, contentType) {
			var now = new Date();
			this.authBuilder.reset().date(now).snDate(true).method(method).url(url);
			if (contentType) {
				this.authBuilder.contentType(contentType);
			}
			request$$1.setRequestHeader(solarnetworkApiCore.HttpHeaders.X_SN_DATE, this.authBuilder.requestDateHeaderValue);
			request$$1.setRequestHeader(solarnetworkApiCore.HttpHeaders.AUTHORIZATION, this.authBuilder.buildWithSavedKey());
		}
   ...
   */
		...
		value: function deferJsonRequestWithAuth(q, method, url) {
			...
			if (method !== solarnetworkApiCore.HttpMethod.GET) {
				queryIndex = url.indexOf('?');
				reqData = url.substring(queryIndex + 1);
				contentType = solarnetworkApiCore.HttpContentType.FORM_URLENCODED_UTF8;
			}
			var req = d3Request.request(queryIndex >= 0 ? url.substring(0, queryIndex) : url).mimeType(solarnetworkApiCore.HttpContentType.APPLICATION_JSON).on('beforesend', function (request$$1) {
				_this.handleRequestAuth(request$$1, method, url, contentType);
			});
   ...
   */
		...
		value: function value(desiredValue) {
			...
			if (pendingState === solarnetworkApiCore.InstructionStates.Queued && pendingValue !== desiredValue) {
				// cancel the pending instruction
				solarnetworkApiCore.Logger.debug('Canceling %d pending control %s switch to %s', instrUrlHelper.nodeId, controlId, pendingValue);
				var cancelInstructionUrl = instrUrlHelper.updateInstructionStateUrl(this.lastKnownInstruction.id, solarnetworkApiCore.InstructionStates.Declined);
				this.deferJsonRequestWithAuth(q, solarnetworkApiCore.HttpMethod.POST, cancelInstructionUrl);
				this.lastKnownInstruction = undefined;
				pendingState = undefined;
				pendingValue = undefined;
			}
			if (currentValue !== desiredValue && pendingValue !== desiredValue) {
				solarnetworkApiCore.Logger.debug('Request %d to change control %s to %d', instrUrlHelper.nodeId, controlId, desiredValue);
				var queueInstructionUrl = instrUrlHelper.queueInstructionUrl(SetControlParameterInstructionName, [{ name: controlId, value: String(desiredValue) }]);
				this.deferJsonRequestWithAuth(q, solarnetworkApiCore.HttpMethod.POST, queueInstructionUrl);
			}
			q.awaitAll(function (error, results) {
				if (error) {
					solarnetworkApiCore.Logger.error('Error updating %d control toggler %s: %s', instrUrlHelper.nodeId, controlId, error.status);
   ...
   */
		...
		value: function update() {
			...
			this.deferJsonRequestWithAuth(q, solarnetworkApiCore.HttpMethod.GET, mostRecentUrl);
			...
			this.deferJsonRequestWithAuth(q, solarnetworkApiCore.HttpMethod.GET, viewPendingUrl);
			...
			if (this.lastKnownInstruction && !InstructionFinishedStates.has(this.lastKnownInstructionState())) {
				...
				this.deferJsonRequestWithAuth(q, solarnetworkApiCore.HttpMethod.GET, viewInstructionUrl);
			}

			q.awaitAll(function (error, results) {
				if (error) {
					solarnetworkApiCore.Logger.error('Error querying %d control toggler %s status: %s', instrUrlHelper.nodeId, controlId, error.status);
					...
					if (newValue !== currValue) {
						solarnetworkApiCore.Logger.debug('Current %d control %s value is %s', instrUrlHelper.nodeId, controlId, newValue !== undefined ? newValue : 'N/A');
						...
						_this3.lastKnownInstruction = execInstruction ? execInstruction : pendingInstruction;
...
exports.ControlToggler = ControlToggler;

Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=solarnetwork-control-toggler.js.map
