// https://github.com/SolarNetwork/sn-control-toggler-js Version 0.2.1-dev.0. Copyright 2019 SolarNetwork Foundation.
(function (global, factory) {
  ...
  (global = global || self, factory(global.sn = {}, global.d3, global.d3, global.sn));
  ...
  function () {
    ...
    function ControlToggler(urlHelper, authBuilder, controlId, queryUrlHelper) {
      ...
      this.queryUrlHelper = queryUrlHelper || new solarnetworkApiCore.NodeDatumUrlHelper(urlHelper.environment); // force the nodeId / sourceId to our controlId
    ...
    _createClass(ControlToggler, [{
      ...
      value: function handleRequestAuth(request, method, url, contentType) {
        ...
        request.setRequestHeader(solarnetworkApiCore.HttpHeaders.X_SN_DATE, this.authBuilder.requestDateHeaderValue);
        request.setRequestHeader(solarnetworkApiCore.HttpHeaders.AUTHORIZATION, this.authBuilder.buildWithSavedKey());
      }
      /**
      ...
      value: function deferJsonRequestWithAuth(q, method, url) {
        ...
        var req = d3Request.request(queryIndex >= 0 ? url.substring(0, queryIndex) : url).mimeType(solarnetworkApiCore.HttpContentType.APPLICATION_JSON).on("beforesend", function (request) {
      ...
      value: function update() {
        ...
        var q = d3Queue.queue(); // query for most recently available datum for control to check control value

        var mostRecentUrl = queryUrlHelper.mostRecentDatumUrl();
        this.deferJsonRequestWithAuth(q, solarnetworkApiCore.HttpMethod.GET, mostRecentUrl); // query for pending instructions to see if we have an in-flight SetControlParameter on the go already
